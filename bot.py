...
proxies = {
           'http': 'http://hqzehlje-rotate:u4wx2e7ue9zd@p.webshare.io:80/',
           'https': 'http://hqzehlje-rotate:u4wx2e7ue9zd@p.webshare.io:80/'
}
...
async def ch(message: types.Message):
    ...
    try:
        ...
        if len(cc) == 0:
            return await message.reply("<b>No hay tarjeta para checkar</b>")
        ...
        if len(ccn) < 15 or len(ccn) > 16:
            return await message.reply('<b>Fallo al identificar la tarjeta</b>\n'
                                       '<b>Reason: Formato invalido!</b>')   
        BIN = ccn[:6]
